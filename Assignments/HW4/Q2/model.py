...
def get_attentions(attn_type, H_i, H_j, weight = None, bias = None, bilinear = None):
  '''
  ...
  inputs:
    ...
    bias--the bias if we're using cosine_bias
  ...
  ################################
  #### Q 2.1 #####################
  ## Here, you will implement dot-product attention
  ## It will be very useful here to 
  #
  # !!! CODE GOES HERE:
    # assert(False) ## remove this line once you do the questions
    alpha = H_i.matmul(H_j)
  #
  ##
  ################################
  ##### END ######################
  ...
  ################################
  #### HINT ######################
  ## see cosine attention here for
  ## hints about other parts
    alpha = H_i.matmul(H_j) / (eps_0 + H_i.norm(dim=-1)*(H_j).norm(dim=-2)).unsqueeze(3)
  ################################
  ##### END ######################
    
    
  elif attn_type == 'cosine_bias':
    
    
  ################################
  #### Q 2.2 #####################
  ## Here, you will implement cosine attention
  ## with bias
  # 
  # !!! CODE GOES HERE:
    alpha = H_i.matmul(H_j) / (eps_0 + H_i.norm(dim=-1)*(H_j).norm(dim=-2)).unsqueeze(3) + bias
  #
  ##
  ################################
  ##### END ######################
  ...
  elif attn_type == 'cosine_and_weight':
    
    
  ################################
  #### Q 2.3 #####################
  ## Here, you will implement cosine attention
  ## with an additive weight term 
  ## 
  # !!! CODE GOES HERE:
    alpha = H_i.matmul(H_j) / (eps_0 + H_i.norm(dim=-1)*(H_j).norm(dim=-2)).unsqueeze(3) + H_i.matmul(weight)
  #
  ##
  ################################
  ##### END ######################
  ...
  ## 
  # !!! CODE GOES HERE:
    alpha = (H_i + H_j.permute(0,1,3,2)).matmul(weight)
  #
  ##
  ################################
  ##### END ######################
  elif attn_type == 'bilinear':
    alpha = bilinear(H_i.squeeze(2), H_j.squeeze(3))
  else:
    assert(False)
    
    
  ## !!! you still need to implement attention for this mode!
...
class DecoderRNN(nn.Module):
  ...
  def __init__(self, vocab_size, embed_size, hidden_size, *, enc_attn=True, dec_attn=True,
    ...
    assert(enc_hidden_size == hidden_size)
    ...
    if enc_attn:
      ...
      self.enc_attn_bias = nn.Parameter(torch.rand(1))
      self.enc_attn_weight = nn.Parameter(torch.rand(1,1,self.hidden_size,1))
      #self.enc_bilinear = nn.Parameter(torch.rand(1,1,self.hidden_size, self.hidden_size))
    ...
    if dec_attn:
      ...
      self.dec_attn_bias = nn.Parameter(torch.rand(1))
      self.dec_attn_weight =  nn.Parameter(torch.rand(1,1,self.hidden_size,1))
      #self.dec_bilinear = nn.Parameter(torch.rand(self.hidden_size, self.hidden_size))
  ...
  def forward(self, embedded, hidden, encoder_states=None, decoder_states=None, coverage_vector=None, *,
    ...
    if self.enc_attn or self.pointer:
      ...
      H_i = hidden.expand(num_enc_steps, batch_size, -1).contiguous().unsqueeze(2)
      H_j = encoder_states.unsqueeze(3)
    
      enc_energy = get_attentions(self.attn_type, H_i, H_j, weight = self.enc_attn_weight, bias=self.enc_attn_bias, bilinear = self.enc_bilinear)
      ...
      '''
      hidden_ = hidden.expand(num_enc_steps, batch_size, -1).contiguous().unsqueeze(2)
      encoder_states_ = encoder_states.unsqueeze(3)
        
      enc_energy = hidden_.matmul(self.enc_bilinear).matmul(encoder_states_).squeeze().unsqueeze(2)
      '''
    ...
    if self.dec_attn:
      if decoder_states is not None and len(decoder_states) > 0:
        ...
        H_i = hidden.expand(num_enc_steps, batch_size, -1).contiguous().unsqueeze(2)
        H_j = decoder_states.unsqueeze(3)
        
        dec_energy = get_attentions(self.attn_type, H_i, H_j, weight = self.dec_attn_weight, bias = self.dec_attn_bias, bilinear = self.dec_bilinear)
        
        #dec_energy = hidden_.matmul(decoder_states_).squeeze().unsqueeze(2)
        #dec_energy = hidden_.matmul(self.enc_bilinear).matmul(decoder_states_).squeeze().unsqueeze(2)
        '''
