...
def eval_bs_batch(batch: Batch, model: Seq2Seq, vocab: Vocab, *, pack_seq=True, beam_size=4,
  ...
  if details:
    ...
    file_content = None
  if False: # batch.examples[0].tgt is not None:  # run ROUGE if gold standard summary exists
  ...
  return scores, file_content, decoded_batch
...
def eval_bs(test_set: Dataset, vocab: Vocab, model: Seq2Seq, params: Params):
  ...
  for i in prog_bar:
    batch = next(test_gen)
    scores, file_content, decoded_batch = eval_bs_batch(batch, model, vocab, pack_seq=params.pack_seq,
    ...
    out_strings += [' '.join(s[1:-1]) for s in decoded_batch]
  ...
  if len(params.test_gen_path) > 0:
    with open(params.test_gen_path,'w') as f:
